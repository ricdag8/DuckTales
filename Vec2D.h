...
namespace DT
{
    ...
    struct Vec2D
    {
        ...
        // methods
        T mag() const { return T(std::sqrt(x * x + y * y)); } // Norma vettore
        T mag2() const { return x * x + y * y; } // Norma quadra
        T versX() const { return x ? x / std::abs(x) : 0; } // Versore ix (x/abs(x) == sign(x))
        T versY() const { return y ? y / std::abs(y) : 0; } // Versore iy
        Vec2D  norm() const { T r = 1 / mag(); return Vec2D(x * r, y * r); } // Restituisce vettore normalizzato
        Vec2D  perp() const { return Vec2D(-y, x); } // Restituisce vettore perpendicolare
